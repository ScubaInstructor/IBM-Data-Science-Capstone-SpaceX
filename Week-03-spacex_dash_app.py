...
spacex_df = pd.read_csv("Week-03-spacex_launch_dash.csv")
max_payload = spacex_df['Payload Mass (kg)'].max()
min_payload = spacex_df['Payload Mass (kg)'].min()
launch_names = {0: "success", 1: "failure"}
...
def update_graph(site_dropdown):
    if site_dropdown == 'All Sites':
        ...
        fig = px.pie(df, names=df['class'].map(launch_names), hole=.2, title='Launches for site ' + site_dropdown)
    return fig
